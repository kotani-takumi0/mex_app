...
AI時代に「本当に技術を理解しているか」を証明するポートフォリオプラットフォームです。

**Overview**
MEX App は、AIで開発したコードを「自分の学び」に変えるためのアプリです。
MCPサーバーで開発過程を自動記録し、コミットごとにNotionへ教育的ドキュメント（WHY・HOWの解説 + 基礎〜応用Q&A）を自動生成します。
ユーザーはそのドキュメントで技術を理解し、学んだことを自分の言葉でMEX Appに記録します。
これにより、面接官は候補者がAIに書かせただけなのか、技術を本当に理解しているのかを判別できます。

**Key Features**
- 開発ログ自動記録（MCPサーバーによるAI開発過程の自動キャプチャ）
- 技術ドキュメント自動生成（`/document` コマンドでNotionにWHY/HOW/Q&A付きドキュメントを生成）
- 自分の言葉で記録（ドキュメントで学んだ内容を開発ログとして自分の言葉で記入）
- 公開ポートフォリオ（「完成物 + 過程 + 理解の記録」を企業に提示）
...
| 公開プロジェクト詳細 | `/p/:username/:projectId` | 開発ログと学習記録の公開表示 |
